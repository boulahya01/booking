...
-- Named reservations are additive to the legacy reserved_spots counter so
-- existing matches keep their capacity semantics without destructive rewrites.
...
create function public.get_pitch_availability(
  p_pitch_id uuid
)
returns table (
  booking_id uuid,
...
begin
  ...
  select
    ...
    case
      when m.status = 'active' and m.visibility = 'open'
        then greatest(
          v_pitch.capacity
          - 1
          - coalesce(m.reserved_spots, 0)
          - coalesce(reservation_stats.named_reserved, 0)
          - coalesce(participant_stats.joined_count, 0),
          0
        )
      else 0
...
begin
  ...
  left join lateral (
    select
      count(*)::integer as named_reserved,
      coalesce(bool_or(mr.profile_id = v_uid), false) as reserved_by_me
    from public.match_reservations mr
    where mr.match_id = m.id
  ) reservation_stats on true
  where b.id = p_booking_id
    and (b.status = 'scheduled' or b.user_id = v_uid)
  limit 1;
end;
$$;
...
begin
  ...
  from (
    select
      'organizer:' || o.id::text as entry_id,
      null::uuid as reservation_id,
      o.id as user_id,
      o.full_name as display_name,
      o.username,
      ...
      0 as sort_order
    from public.bookings b
    join context c on c.booking_id = b.id
    join public.profiles o on o.id = b.user_id

    union all

    select
      'player:' || mp.user_id::text,
      null::uuid,
      player.id,
      player.full_name,
      player.username,
    ...
    join public.profiles player on player.id = mp.user_id
    where c.visibility = 'open' or c.organizer_id = v_uid

    union all

    select
      'reservation:' || mr.id::text,
      mr.id,
      reserved_profile.id,
      coalesce(reserved_profile.full_name, mr.guest_name),
      reserved_profile.username,
      'reserved'::text,
      case when mr.profile_id is null then 'guest' else 'registered' end,
      mr.created_at,
      2
    from context c
    join public.match_reservations mr on mr.match_id = c.match_id
...
begin
  ...
  if (nullif(btrim(p_username), '') is null) = (nullif(btrim(p_guest_name), '') is null) then
  ...
  if nullif(btrim(p_username), '') is not null then
    ...
    if exists (
      ...
      where mp.match_id = p_match_id and mp.user_id = v_profile_id
    ) then
      raise exception 'reserved_user_already_in_match';
    end if;

    if exists (
      select 1 from public.match_reservations mr
      where mr.match_id = p_match_id and mr.profile_id = v_profile_id
    ) then
  ...
  select count(*)::integer into v_named_reserved
...
begin
  ...
  select b.starts_at into v_starts_at
  from public.bookings b
  where b.id = v_match.booking_id;
...
begin
  ...
  select
    ...
    case
      when m.organizer_id = v_uid then 'organizer'
      when exists (
        select 1 from public.match_participants mp
