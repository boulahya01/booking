...
if (!supabaseServiceKey) {
  console.error("[admin-promote] Missing SUPABASE_SERVICE_ROLE_KEY or SERVICE_ROLE_KEY");
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!admin || admin.role !== "admin") {
      console.error(`[admin-promote] Access denied - user ${payload.id} is not admin`);
    ...
    // Parse request body
    let body: PromoteRequest = { role: "user" };
    try {
    ...
    console.log(`[admin-promote] Admin ${payload.id} promoting user ${userId} to ${body.role}`);
    ...
    if (updateError) {
      console.error(`[admin-promote] Update error:`, updateError);
      return new Response(JSON.stringify({ message: "Failed to promote user" }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
    ...
    console.log(`[admin-promote] User ${userId} promoted to ${body.role} in ${Date.now() - startTime}ms`);
