...
if (!supabaseServiceKey) {
  console.error("[admin-reject] Missing SUPABASE_SERVICE_ROLE_KEY or SERVICE_ROLE_KEY");
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!admin || admin.role !== "admin") {
      console.error(`[admin-reject] Access denied - user ${payload.id} is not admin`);
    ...
    console.log(`[admin-reject] Admin ${payload.id} rejecting user ${userId}`);
    ...
    if (updateError) {
      ...
      return new Response(JSON.stringify({ message: "Failed to reject user" }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
    ...
    console.log(`[admin-reject] User ${userId} rejected in ${Date.now() - startTime}ms`);
