...
if (!supabaseServiceKey) {
  console.error("[admin-pending-users] Missing SUPABASE_SERVICE_ROLE_KEY or SERVICE_ROLE_KEY");
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!user || (user.role !== "admin" && user.role !== "owner")) {
      console.error(`[admin-pending-users] Access denied - user ${payload.id} is not admin/owner`);
    ...
    console.log(`[admin-pending-users] Admin ${payload.id} fetching pending users`);
    ...
    if (pendingError) {
      console.error(`[admin-pending-users] Query error:`, pendingError);
      return new Response(JSON.stringify({ message: "Failed to fetch pending users" }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
    ...
    const response = (pendingUsers || []).map((u: any) => ({
      id: u.id,
      email: u.email,
    ...
    console.log(`[admin-pending-users] Retrieved ${response.length} pending users in ${Date.now() - startTime}ms`);
