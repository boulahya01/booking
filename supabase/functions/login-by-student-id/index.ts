...
serve(async (req: Request): Promise<Response> => {
    ...
    try {
        ...
        // Step 1: Find profile by student_id (normalized to uppercase)
        ...
        // Validate email format to prevent bounce-backs
        if (!isValidEmail(userData.user.email)) {
            console.error("Invalid email format:", userData.user.email)
            return new Response(
                JSON.stringify({ error: "Invalid email format on account" }),
                {
                    status: 400,
                    headers: { "Content-Type": "application/json" },
                }
