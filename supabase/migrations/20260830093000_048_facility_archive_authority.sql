-- Preserve the dedicated, reasoned facility archive boundary.
--
-- Existing active facilities must be deactivated through admin_archive_pitch(),
-- which records a structured reason in the audit log. admin_save_pitch() remains
-- authoritative for configuration edits and may reactivate an inactive facility.
...
begin
  ...
  if p_pitch_id is null then
    ...
    -- Deactivation is a distinct audited operation. Holding the pitch row before
    -- this check also prevents save/archive races from bypassing the reasoned
    -- transition. Restoring an inactive facility remains an ordinary edit.
