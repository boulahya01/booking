-- Keep facility schedule edits consistent with existing booking commitments.
--
-- create_booking() holds a shared lock on the pitch while validating and inserting
-- a booking. admin_save_pitch() holds an exclusive lock on the same row, so this
-- guard also serializes schedule changes against concurrent booking creation.
-- Existing bookings keep their recorded duration; only proposed timezone/opening
-- hours are checked so grandfathered slot-duration changes remain possible.
    ...
    then
      if exists (
        ...
        where b.pitch_id = p_pitch_id
          ...
          and (
            b.starts_at < (
              ((b.starts_at at time zone v_timezone)::date::timestamp + p_open_time)
              at time zone v_timezone
            )
            or b.ends_at > (
              ((b.starts_at at time zone v_timezone)::date::timestamp + p_close_time)
              at time zone v_timezone
            )
          )
      ) then
