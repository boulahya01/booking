create or replace function public.create_open_match(
  p_booking_id uuid,
  p_reserved_spots integer default 0
)
...
begin
  ...
  if v_booking.status <> 'scheduled' or v_booking.starts_at <= now() then
    raise exception 'booking_not_matchable';
  end if;

  if v_capacity < 2 then
    raise exception 'match_capacity_too_small';
  end if;

  if p_reserved_spots < 0 or p_reserved_spots > v_capacity - 1 then
    raise exception 'invalid_reserved_spots';
  end if;
  ...
  if found then
    ...
    set visibility = 'open',
        reserved_spots = p_reserved_spots,
        status = 'active',
        updated_at = now()
    where id = v_match.id
