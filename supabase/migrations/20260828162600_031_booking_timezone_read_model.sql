-- Include facility timezone in authoritative booking read models so clients do
-- not need a second pitches query after scoped booking RPCs.
begin;
...
begin
  ...
  return query
  select
    b.id,b.pitch_id,b.starts_at,b.ends_at,b.status,
    case
    ...
    b.cancelled_at,b.created_at,p.name,p.location,p.capacity,p.timezone
...
drop function if exists public.get_next_booking();

create function public.get_next_booking()
returns table(
  booking_id uuid,
  pitch_id uuid,
  starts_at timestamptz,
...
begin
  ...
  select
    b.id,b.pitch_id,b.starts_at,b.ends_at,b.status,
    case when b.starts_at<=now() then 'in_progress' else 'upcoming' end,
    b.cancelled_at,b.created_at,p.name,p.location,p.capacity,p.timezone
  from public.bookings b
  join public.pitches p on p.id=b.pitch_id
  where b.user_id=v_uid
