...
export function Home() {
  ...
  const fetchAvailableSlots = async (pitchId: string) => {
    try {
      // Try to read cached slots for fast UI display (valid for 2 minutes)
      const cacheKey = `slots_cache:${pitchId}`
      let usedCache = false
      try {
        const raw = localStorage.getItem(cacheKey)
        if (raw) {
          const parsedCache = JSON.parse(raw)
          if (parsedCache && Array.isArray(parsedCache) && (Date.now() - parsedCache.ts) < 2 * 60 * 1000) {
      ...
      // Save enriched results to cache for fast subsequent displays
      try {
        const cacheKey = `slots_cache:${pitchId}`
        localStorage.setItem(cacheKey, JSON.stringify({ slots: enriched, ts: Date.now() }))
      } catch (_e) {
