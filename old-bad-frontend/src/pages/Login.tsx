  ...
  const handleStudentIdLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/login-by-student-id', {
        ...
        body: JSON.stringify({ studentId: normalizedStudentId, password }),
      })
      ...
      <Card>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          {loginMode === 'email' ? (
            ...
            <Input id="studentId" type="text" value={studentId} onChange={(e) => setStudentId(e.target.value.replace(/\s+/g, '').toUpperCase())} placeholder={t('login.student_id')} disabled={loading} icon={<FiUser />} />
          )}
