...
export function AdminUsers() {
  ...
  const openIdPhoto = async (path?: string) => {
    if (!path) return
    try {
      ...
      console.error('openIdPhoto error', err)
      toast.error(err?.message || 'Failed to open ID photo')
    }
  }
          ...
          {pendingUsers.map((user) => (
            <div key={user.id} className="user-card">
              <div className="user-info">
                ...
                <div className="user-verification-status">Verification: {user.verification_status || 'unsubmitted'}</div>
              ...
              <div className="user-actions">
                {user.id_photo_url && (
                  <button onClick={() => openIdPhoto(user.id_photo_url)} className="btn-view-id" title="View ID photo">
                    View ID
                  </button>
                )}
                <button
