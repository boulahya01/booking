...
export type MySupportThreadSummary = Pick<SupportThread, 'id' | 'kind' | 'status' | 'subject' | 'created_at'> & {
  updated_at: string
...
export async function createAuthenticatedSupportThread(input: { kind: Exclude<SupportKind, 'report'>; subject?: string; body: string }): Promise<string> {
  const { data, error } = await supabase.rpc('create_my_support_thread', { p_kind: input.kind, p_subject: input.subject ?? '', p_body: input.body })
  if (error) throw normalizeError(error)
  return data as string
}

export async function createAuthenticatedReportThread(input: { targetType: ReportTargetType; targetId: string; reason: ReportReason; body: string }): Promise<string> {
  const { data, error } = await supabase.rpc('create_my_report_thread', { p_target_type: input.targetType, p_target_id: input.targetId, p_reason_code: input.reason, p_body: input.body })
  if (error) throw normalizeError(error)
  return data as string
}
...
export async function listMySupportThreads(): Promise<MySupportThreadSummary[]> {
  ...
  return (Array.isArray(data) ? data : []).map((row: any) => ({
    ...
    last_message: row.last_message_id ? { id: row.last_message_id, sender_role: row.last_sender_role as SupportMessage['sender_role'], body: row.last_body, created_at: row.last_message_at } : null
  }))
}

export async function getMySupportThread(threadId: string): Promise<SupportThread | null> {
  const { data, error } = await supabase.rpc('get_my_support_thread', { p_thread_id: threadId })
  if (error) {
  ...
  return {
    ...
    messages: data.map((row: any) => ({ id: row.message_id, sender_role: row.sender_role as SupportMessage['sender_role'], body: row.body, created_at: row.message_created_at }))
  }
}

export async function addAuthenticatedSupportMessage(threadId: string, body: string): Promise<void> {
  const { error } = await supabase.rpc('add_my_support_message', { p_thread_id: threadId, p_body: body })
  if (error) throw normalizeError(error)
}

export async function createGuestSupportThread(input: { contactEmail?: string; subject?: string; body: string }): Promise<{ threadId: string; accessToken: string }> {
  let response: Response
  try {
    response = await fetch('/api/support/guest', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ contactEmail: input.contactEmail ?? '', subject: input.subject ?? '', body: input.body })
    })
  } catch {
  ...
  try { payload = await response.json() } catch { /* keep proxy failures generic */ }
  if (!response.ok) throw new Error(typeof payload?.error === 'string' ? payload.error : 'Support is temporarily unavailable. Please try again.')
  if (!payload?.threadId || !payload?.accessToken) throw new Error('Support is temporarily unavailable. Please try again.')
...
export async function getGuestSupportThread(accessToken: string): Promise<SupportThread | null> {
  ...
  return {
    ...
    messages: data.map((row: any) => ({ id: row.message_id, sender_role: row.sender_role, body: row.body, created_at: row.message_created_at }))
  }
}

export async function addGuestSupportMessage(accessToken: string, body: string): Promise<void> {
  const { error } = await supabase.rpc('add_guest_support_message', { p_access_token: accessToken, p_body: body })
  if (error) throw normalizeError(error)
}

export async function listAdminSupportThreads(status: SupportStatus | null = null): Promise<SupportThreadSummary[]> {
  const { data, error } = await supabase.rpc('admin_list_support_threads', { p_status: status, p_limit: 50 })
  if (error) throw normalizeError(error)
...
export async function adminReplySupportThread(threadId: string, body: string, nextStatus: SupportStatus = 'waiting'): Promise<void> {
  const { error } = await supabase.rpc('admin_reply_support_thread', { p_thread_id: threadId, p_body: body, p_next_status: nextStatus })
  if (error) throw normalizeError(error)
}

export async function adminSetSupportStatus(threadId: string, status: SupportStatus): Promise<void> {
  const { error } = await supabase.rpc('admin_set_support_status', { p_thread_id: threadId, p_status: status })
  if (error) throw normalizeError(error)
}
