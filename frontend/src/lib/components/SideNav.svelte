...
{#if isOpen}
  <div class="fixed inset-0 z-40 bg-black/50 backdrop-blur-[2px]" on:click={() => dispatch('close')} role="presentation"></div>
{/if}

<nav
  class="uneem-side-nav fixed inset-y-0 z-50 w-[17.5rem] max-w-[86vw] border-e border-border-light bg-surface-raised shadow-xl transition-transform duration-200 ease-out"
...
>
  <div class="flex h-full flex-col">
    <div class="flex min-h-[56px] items-center justify-between px-4">
      <button on:click={() => navigate('/home')} class="min-h-10 text-[17px] font-extrabold tracking-[0.14em] text-text">UNEEM</button>
      <button on:click={() => dispatch('close')} class="grid h-10 w-10 place-items-center rounded-xl text-text-secondary hover:bg-surface-level-1 hover:text-text" aria-label={$language === 'ar' ? 'إغلاق' : 'Close menu'}>
        <Icon name="x" size={19} />
      </button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-3 py-2">
      <div class="space-y-0.5">
        {#each regularItems as item}
          <button
            ...
            class="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-start text-sm font-semibold transition-colors"
            class:bg-surface-level-1={isActive(item.href)}
      ...
      {#if $needsIdentityAction}
        <button on:click={() => navigate('/verification')} class="mt-4 flex w-full items-center gap-3 rounded-xl bg-warning-light px-3 py-3 text-start text-warning">
          <Icon name="shield" size={18} />
          <span class="text-sm font-semibold">{$language === 'ar' ? 'أكمل التحقق' : 'Finish verification'}</span>
        </button>
      {/if}

      {#if $isAdmin}
        <div class="my-4 h-px bg-border-light"></div>
        <button
          on:click={() => navigate('/admin')}
          class="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-start text-sm font-semibold transition-colors"
          class:bg-surface-level-1={isActive('/admin')}
        ...
        >
          ...
          <Icon name={$language === 'ar' ? 'chevron-left' : 'chevron-right'} size={16} className="ms-auto opacity-55" />
        </button>
      {/if}
    </div>

    <div class="space-y-0.5 border-t border-border-light p-3">
