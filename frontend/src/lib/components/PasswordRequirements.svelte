<script lang="ts">
  ...
  export let password = ''
  export let lengthLabel = '8 characters minimum'
  export let numberOrSymbolLabel = '1 number or symbol'

  $: requirements = passwordRequirements(password)
  $: rows = [
    { label: lengthLabel, passed: requirements.length },
    { label: numberOrSymbolLabel, passed: requirements.numberOrSymbol }
  ]
</script>
...
<ul class="space-y-2 px-1" aria-label="Password requirements" aria-live="polite">
  {#each rows as row}
    <li class={`flex items-center gap-2 text-sm ${row.passed ? 'text-success' : password.length ? 'text-text-secondary' : 'text-text-muted'}`}>
      <span
        class={`grid h-5 w-5 shrink-0 place-items-center rounded-full ${row.passed ? 'bg-success-light text-success' : 'bg-surface-level-1 text-text-muted'}`}
        aria-hidden="true"
      >
