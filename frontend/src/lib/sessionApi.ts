...
export type BootstrapError = 'profile_not_found' | 'network_error' | 'database_error'
...
export async function getMySessionContextOrError(): Promise<SessionContext | { error: BootstrapError }> {
  try {
    const { data, error } = await supabase.rpc('get_my_session_context')
    if (error) {
