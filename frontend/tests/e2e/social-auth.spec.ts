...
for (const provider of ['facebook','google']) {
  test(`${provider} delegates authentication to Supabase PKCE without layout shift`,async({page})=>{
    let release!:()=>void
    let reads=0
    await page.route('**/auth/v1/settings',async route=>{
      reads++
      await new Promise<void>(resolve=>{release=resolve})
      await route.fulfill({contentType:'application/json',body:JSON.stringify({external:{google:true,facebook:true}})})
    })
    ...
    const button=page.getByRole('button',{name:provider==='google'?'Google':'Facebook',exact:true})
    ...
    expect(reads).toBe(1)
    await page.screenshot({path:test.info().outputPath(`login-${provider}.png`),fullPage:true})
    await button.click()
    await expect(page.getByRole('heading',{name:'Provider authorization boundary'})).toBeVisible()
    expect(authUrl!.searchParams.get('provider')).toBe(provider)
    expect(authUrl!.searchParams.get('redirect_to')).toBe(new URL('/verify-email',test.info().project.use.baseURL).href)
    expect(authUrl!.searchParams.get('code_challenge_method')).toBe('s256')
    expect(authUrl!.searchParams.get('code_challenge')).toBeTruthy()
  })
}

test('unavailable providers leave email sign-in usable',async({page})=>{
  ...
  await expect(page.getByRole('button',{name:'Facebook',exact:true})).toBeDisabled()
